...
:information_source: In case of any issues during installation, please read [FAQ section](#faq)
 ...
 1. Download or clone the project with DevBox configuration:
    ...
    Optionally, if you use private repositories on GitHub or download packages from the Magento Marketplace using Composer.
     ...
     1. Copy (optional) [etc/instance/config.yaml.dist](etc/instance/config.yaml.dist) as `etc/config.yaml` and make the necessary customizations.
...
Some of default settings are available for override. These settings can be found in the file [etc/instance/config.yaml.dist](etc/instance/config.yaml.dist).
...
- Path to your Magento installation in the container:
  - Can be retrieved from environment variable: `echo ${DEVBOX_ROOT}/$(getContext)`
...
Hostname can also be found in `maghento/host_name` section of [config.yaml](etc/instance/config.yaml.dist).
...
Make sure that `ce_sample_data` and `ee_sample_data` are defined in [config.yaml](etc/instance/config.yaml.dist) and point CE and optionally EE sample data repositories.
...
To disable this feature, set `magento/generate_basic_data` in [config.yaml](etc/instance/config.yaml.dist) to `0` and run `./m-switch-to-ce -f` or `./m-switch-to-ee -f`, depending on the edition to be installed.
...
You can also set `use_varnish: 1` in [config.yaml](etc/instance/config.yaml.dist) to use varnish. Changes will be applied on `init_project.sh -f`.

The VCL content can be found in [configmap.yaml](etc/helm/templates/configmap.yaml).

### Activating ElasticSearch

Set `search_engine: "elasticsearch"` in [config.yaml](etc/instance/config.yaml.dist) to use ElasticSearch as current search engine or `search_engine: "mysql"` to use MySQL. Changes will be applied on `m-reinstall`.
...
### FAQ
 1. To debug any CLI script in current Devbox project, set `debug:devbox_project` option in [config.yaml](etc/instance/config.yaml.dist) to `1`
