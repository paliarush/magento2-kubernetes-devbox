...
function assertTestsConfigured()
{
    ...
    # Unit tests
    unit_tests_config_path="${devbox_dir}/$(getContext)/dev/tests/unit/phpunit.xml"
    assertTrue "Unit tests are not configured ('${unit_tests_config_path}' is missing)" '[[ -f ${unit_tests_config_path} ]]'
    
    # Integration tests
    integration_tests_config_path="${devbox_dir}/$(getContext)/dev/tests/integration/phpunit.xml"
    assertTrue "Integration tests are not configured ('${integration_tests_config_path}' is missing)" '[[ -f ${integration_tests_config_path} ]]'
    integration_tests_mysql_config_path="${devbox_dir}/$(getContext)/dev/tests/integration/etc/install-config-mysql.php"
    ...
    # REST Web API tests
    rest_tests_config_path="${devbox_dir}/$(getContext)/dev/tests/api-functional/phpunit_rest.xml"
    ...
    # SOAP Web API tests
    soap_tests_config_path="${devbox_dir}/$(getContext)/dev/tests/api-functional/phpunit_soap.xml"
    ...
    graphql_tests_config_path="${devbox_dir}/$(getContext)/dev/tests/api-functional/phpunit_graphql.xml"
    ...
    # Functional tests
    functional_tests_config_path="${devbox_dir}/$(getContext)/dev/tests/functional/phpunit.xml"
    ...
    functional_tests_config_path="${devbox_dir}/$(getContext)/dev/tests/functional/etc/config.xml"
    assertTrue "Functional tests are not configured ('${functional_tests_config_path}' is missing)" '[[ -f ${functional_tests_config_path} ]]'
    functional_tests_config_content="$(cat "${functional_tests_config_path}")"
    pattern="${current_magento_base_url}"
    assertTrue "Contents of '${functional_tests_config_path}' seems to be invalid ${functional_tests_config_content} =~ ${pattern}" '[[ ${functional_tests_config_content} =~ ${pattern} ]]'
...
function assertDebugConfigurationWork()
{
    ...
    cd "${devbox_dir}"
    sed -i.back 's|magento_storefront: 0|magento_storefront: 1|g' "${devbox_dir}/etc/instance/$(getContext).yaml" >>${current_log_file_path} 2>&1
    sed -i.back 's|magento_admin: 0|magento_admin: 1|g' "${devbox_dir}/etc/instance/$(getContext).yaml" >>${current_log_file_path} 2>&1
    ...
    sed -i.back 's|magento_storefront: 1|magento_storefront: 0|g' "${devbox_dir}/etc/instance/$(getContext).yaml" >>${current_log_file_path} 2>&1
    sed -i.back 's|magento_admin: 1|magento_admin: 0|g' "${devbox_dir}/etc/instance/$(getContext).yaml" >>${current_log_file_path} 2>&1
    bash m-clear-cache >>${current_log_file_path} 2>&1
...
function assertRedisCacheIsEnabled()
{
    ...
    cache_directory="${devbox_dir}/$(getContext)/var/cache"
